...
(module $wasmfx_import
  ...
  (func $print-i32 (import "benchmark" "print_i32") (param i32))
  (func $print-cont (import "benchmark" "print_cont") (param (ref null $cfilter)))
  ...
  (func $filter_yield (export "filter_yield") (param $result i32) (result i32)
    (local $i i32)
    (call $print-i32 (i32.const 40))
    (local.set $i (suspend $yield (local.get $result)))
    (call $print-i32 (i32.const 44))
    (return (local.get $i)))
  ...
  (func $filter_spawn (export "filter_spawn") (param $prime i32) (result i32)
    ...
    (block $on_yield (result i32 (ref $cfilter))
      (resume $cfilter (tag $yield $on_yield)
                       ...
                       (local.get $fiber))
      (unreachable)
    ) ;; on_init [ i32 (ref $cfilter) ]
    (local.set $fiber)
    (drop) ;; discard the dummy i32.
  ...
  (func $filter_send (export "filter_send") (param $fiber_idx i32) (param $candidate i32) (result i32)
    (local $next_k (ref null $cfilter))
    (block $on_yield (result i32 (ref $cfilter))
      (call $print-i32 (i32.const 41))
      (local.set $next_k (table.get $conts (local.get $fiber_idx)))
      (block $continue
        (block $if-null
           (br_on_null $if-null (local.get $next_k))
           (call $print-i32 (i32.const 1000))
           (br $continue)
        )
        (call $print-i32 (i32.const 2000))
      )
      (call $print-i32 (i32.const 42))
      (resume $cfilter (tag $yield $on_yield)
                       (local.get $candidate)
                       (local.get $next_k))
      (call $print-i32 (i32.const 128))
      (unreachable)
    ) ;; on_yield [ i32 (ref $cfilter) ]
    (call $print-i32 (i32.const 43))
    (local.set $next_k)
