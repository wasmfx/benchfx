...
extern
__wasm_import__("benchmark", "async_worker")
void* async_worker(void*);

__noinline
__wasm_export__("alloc_async_worker")
void alloc_async_worker(uint32_t key) {
  store[key] = fiber_alloc(async_worker);
}

__noinline
__wasm_export__("resume_async_worker")
